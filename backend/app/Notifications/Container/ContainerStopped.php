...
class ContainerStopped extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject(product_name().": A resource  has been stopped unexpectedly on {$this->server->name}");
    ...
    {
        $message = product_name().": A resource ($this->name) has been stopped unexpectedly on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
