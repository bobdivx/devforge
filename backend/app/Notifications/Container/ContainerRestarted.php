...
class ContainerRestarted extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject(product_name().": A resource ({$this->name}) has been restarted automatically on {$this->server->name}");
    ...
    {
        $message = product_name().": A resource ({$this->name}) has been restarted automatically on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
            $payload['buttons'] = [
                [
                    [
                        'text' => 'Check Proxy in '.product_name(),
                        'url' => $this->url,
                    ],
                ],
            ];
