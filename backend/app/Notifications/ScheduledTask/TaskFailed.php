...
class TaskFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject(product_name().": [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        $message = product_name().": Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in '.product_name(),
                'url' => (string) $this->url,
            ];
        }
