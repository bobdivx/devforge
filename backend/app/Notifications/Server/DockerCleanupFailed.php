...
class DockerCleanupFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject(product_name().": [ACTION REQUIRED] Docker cleanup job failed on {$this->server->name}");
    ...
    public function toDiscord(): DiscordMessage
    {
        return new DiscordMessage(
            title: ':cross_mark: '.product_name().': [ACTION REQUIRED] Docker cleanup job failed on '.$this->server->name,
    ...
    {
        return [
            'message' => product_name().": [ACTION REQUIRED] Docker cleanup job failed on {$this->server->name}!\n\n{$this->message}",
        ];
    }
    ...
    public function toSlack(): SlackMessage
    {
        return new SlackMessage(
            title: product_name().': [ACTION REQUIRED] Docker cleanup job failed',
