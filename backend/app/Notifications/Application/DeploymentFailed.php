...
class DeploymentFailed extends CustomEmailNotification
{
    ...
    public function __construct(Application $application, string $deployment_uuid, ?ApplicationPreview $preview = null)
    {
        ...
        $this->fqdn = preferred_fqdn(data_get($application, 'fqdn'), $application->uuid);
    ...
    public function toMail(): MailMessage
    {
        ...
        if ($pull_request_id === 0) {
            $mail->subject(product_name().': Deployment failed of '.$this->application_name.'.');
        } else {
            $fqdn = $this->preview->fqdn;
            $mail->subject(product_name().': Deployment failed of pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.'.');
        }
    ...
    {
        if ($this->preview) {
            $message = product_name().': Pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.' ('.$this->preview->fqdn.') deployment failed: ';
        } else {
            $message = product_name().': Deployment failed of '.$this->application_name.' ('.$this->fqdn.'): ';
        }
        $buttons[] = [
