...
class ApplicationsController extends Controller
{
                ...
                new OA\MediaType(
                    ...
                    schema: new OA\Schema(
                        ...
                        properties: [
                            ...
                            'is_force_https_enabled' => ['type' => 'boolean', 'description' => 'The flag to indicate if HTTPS is forced. Defaults to false.'],
                            ...
                            'docker_compose_domains' => [
                                ...
                                'items' => new OA\Schema(
                                    ...
                                    properties: [
                                        ...
                                        'domain' => ['type' => 'string', 'description' => 'Comma-separated list of URLs (e.g. "https://example.com,https://app2.example.com")'],
                                    ],
                                ),
                            ],
    ...
    private function create_application(Request $request, $type)
    {
        ...
        $allowedFields = ['project_uuid', 'environment_name', 'environment_uuid', 'server_uuid', 'destination_uuid', 'type', 'name', 'description', 'is_static', 'is_spa', 'is_auto_deploy_enabled', 'is_force_https_enabled', 'domains', 'git_repository', 'git_branch', 'git_commit_sha', 'private_key_uuid', 'docker_registry_image_name', 'docker_registry_image_tag', 'build_pack', 'install_command', 'build_command', 'start_command', 'ports_exposes', 'ports_mappings', 'custom_network_aliases', 'base_directory', 'publish_directory', 'health_check_enabled', 'health_check_type', 'health_check_command', 'health_check_path', 'health_check_port', 'health_check_host', 'health_check_method', 'health_check_return_code', 'health_check_scheme', 'health_check_response_text', 'health_check_interval', 'health_check_timeout', 'health_check_retries', 'health_check_start_period', 'limits_memory', 'limits_memory_swap', 'limits_memory_swappiness', 'limits_memory_reservation', 'limits_cpus', 'limits_cpuset', 'limits_cpu_shares', 'custom_labels', 'custom_docker_run_options', 'post_deployment_command', 'post_deployment_command_container', 'pre_deployment_command', 'pre_deployment_command_container',  'manual_webhook_secret_github', 'manual_webhook_secret_gitlab', 'manual_webhook_secret_bitbucket', 'manual_webhook_secret_gitea', 'redirect', 'github_app_uuid', 'instant_deploy', 'dockerfile', 'dockerfile_location', 'docker_compose_location', 'docker_compose_raw', 'docker_compose_custom_start_command', 'docker_compose_custom_build_command', 'docker_compose_domains', 'watch_paths', 'use_build_server', 'static_image', 'custom_nginx_configuration', 'is_http_basic_auth_enabled', 'is_sso_protected', 'has_own_user_system', 'http_basic_auth_username', 'http_basic_auth_password', 'connect_to_docker_network', 'force_domain_override', 'autogenerate_domain', 'is_container_label_escape_enabled', 'is_preserve_repository_enabled'];
        ...
        $validator = customApiValidator($request->all(), [
            ...
            'is_sso_protected' => 'boolean|nullable',
            'has_own_user_system' => 'boolean|nullable',
    ...
    public function update_by_uuid(Request $request)
    {
        ...
        if (($request->has('is_http_basic_auth_enabled') || $request->has('is_sso_protected') || $request->has('has_own_user_system')) && $application->is_container_label_readonly_enabled === false) {
            if ($request->has('has_own_user_system')) {
                $application->has_own_user_system = $request->has_own_user_system;
            }
            if ($request->has('is_sso_protected')) {
                $application->is_sso_protected = $request->is_sso_protected;
            }
            if ($application->has_own_user_system === true) {
                $application->is_sso_protected = false;
            } elseif ($application->is_sso_protected === true && $application->has_own_user_system === null) {
