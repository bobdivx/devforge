...
/**
 * Fallback empty/absurd: stronger Ollama → agent.fallback_provider → other team providers.
 */
class AgentChatEmptyReplyFallback
{
    public function log(AiAgentRun $run, string $raw, string $reason = 'empty_or_absurd'): void
    {
        ...
        Log::warning('agent.chat.empty_or_absurd_reply', [
            'run_id' => $run->id ?? null, 'preview' => $preview, 'reason' => $reason,
        ]);
    }
    ...
    public function strongerOllamaModel(?string $currentModel, array $available = []): ?string
    {
        ...
        foreach (LlmModelResolver::prioritizeOllamaModelsForTools($available) as $model) {
    ...
    /** @param  array<string, mixed>  $llm @return array<string, mixed>|null */
    ...
    public function orderedFallbackConfigs(AiAgent $agent, array $currentLlm = []): array
    {
        ...
        foreach ($others as $other) {
            if (! $other instanceof AiProviderConfig || in_array((int) $other->id, $usedIds, true)) {
                continue;
            }
            $ordered[] = $other;
