    ...
    public function builtinDefinitions(): array
    {
        return [
            ...
            [
                ...
                'priority' => 170,
                'body' => implode("\n", [
                    '# Deploy Graft to All Repositories',
                    '',
                    'Automation procedure to deploy the Graft context graph to all team repositories via GitHub tools.',
                    '',
                    '## Repositories',
                    ...
                    '- bobdivx/tesla',
                    '',
                    '## Deployment Procedure per Repository',
                    'For each repository:',
                    '1. Check if already configured via read_github_file(repo, "package.json"). If @nanonets/graft is in devDependencies, skip.',
                    '2. Update package.json to add @nanonets/graft to devDependencies.',
                    '3. Create or merge .mcp.json with graft server config (command: npx, args: ["graft", "mcp", "--dir", ".graft"]).',
                    '4. Update .gitignore to include /.graft/.',
                    '5. Create GRAFT.md with usage documentation.',
                    '6. Commit directly on main or open PR if branch is protected.',
                    '',
                    '## Summary Report',
                    'Report total deployed, PRs created, skipped, and errors.',
                ]),
            ],
            [
                ...
                'body' => implode("\n", [
                    ...
                    '',
                    'Graph-based codebase navigation: find symbols, trace calls, and analyze blast radius with minimal token usage.',
                    '',
                    '## Available Tools & Patterns',
                    '- Find symbols and definitions across PHP, TypeScript, and Blade files.',
                    '- Analyze call graph to trace upstream callers and downstream callees before making changes.',
                    '- Inspect API signatures without reading entire file implementations.',
                    '',
                    '## When to Use',
                    '- Diagnosing complex multi-file bugs or refactoring shared services.',
                    '- Evaluating blast radius of API or schema modifications.',
                ]),
            ],
            [
