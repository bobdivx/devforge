...
RULES;
    ...
    private function rankTeamApplications(int $teamId, int $limit): array
    {
        ...
        try {
            $deployments = ApplicationDeploymentQueue::query()
                ...
                ->orderByDesc('id')
                ->get(['id', 'application_id', 'status', 'created_at', 'updated_at', 'finished_at', 'rollback']);
        ...
        foreach ($apps as $application) {
            ...
            if (str_contains($statusLower, 'unhealthy') || str_contains($statusLower, 'failed') || str_contains($statusLower, 'error') || $failedDeploy) {
                $severity = 0;
    ...
    public function formatFleetPromptBlock(array $fleet): string
    {
        ...
        foreach ($fleet as $app) {
            ...
            $deployLabel = is_string($deployStatus) && $deployStatus !== '' ? $deployStatus : 'aucun';
