...
it('keeps the ZimaOS compose copies identical', function () {
    ...
    expect(is_file($import))->toBeTrue()
        ...
        ->and(file_get_contents($import))->not->toContain('AGENT_MODEL')
        ->and(file_get_contents($import))->not->toContain('AGENT_PROVIDER')
        ->and(file_get_contents($import))->not->toContain('AGENT_API_KEY')
        ->and(file_get_contents($import))->not->toContain('AGENT_BASE_URL');
});
...
it('starts DevForge on ZimaOS with web, proxy, and AppData paths', function (string $relativePath) {
    ...
    expect($services)->toHaveKeys(['proxy', 'web', 'api', 'db', 'redis', 'realtime', 'keeper', 'agent']);
    ...
    expect($services['proxy']['ports'][0]['published'])->toBe('8080')
        ...
        ->and($services['api']['depends_on'])->toHaveKeys(['db', 'redis', 'realtime', 'agent'])
    ...
    expect(zimaosEnv($agentEnv, 'AGENT_LISTEN'))->toBe('0.0.0.0:8090')
...
it('ships a ZimaOS App Store compose with pinned tags and /DATA/AppData volumes', function () {
    ...
    expect($compose['name'])->toBe('devforge')
        ...
        ->and($services['web']['image'])->toBe('bobdivx/devforge:web-4.1.8')
        ->and($services['api']['image'])->toBe('bobdivx/devforge:4.1.8')
        ->and($services['realtime']['image'])->toBe('bobdivx/devforge:realtime-4.1.8')
        ->and($services['agent']['image'])->toBe('bobdivx/devforge:agent')
    ...
    expect(zimaosEnv($agentEnv, 'AGENT_LISTEN'))->toBe('0.0.0.0:8090')
        ->and(zimaosEnv($agentEnv, 'AGENT_MODEL'))->toBeNull()
        ->and(zimaosEnv($agentEnv, 'AGENT_PROVIDER'))->toBeNull()
        ->and(file_get_contents($path))->not->toContain('AGENT_MODEL')
        ->and(file_get_contents($path))->not->toContain('gpt-4o-mini');
