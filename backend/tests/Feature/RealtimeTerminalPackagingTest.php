...
it('copies the realtime terminal utilities into the container image', function () {
    $dockerfile = file_get_contents(base_path('docker/devforge-realtime/Dockerfile'));

    expect($dockerfile)->toContain('COPY docker/devforge-realtime/terminal-utils.js /terminal/terminal-utils.js');
});
...
it('mounts the realtime terminal utilities in local development compose files', function (string $composeFile) {
    ...
    expect($composeContents)->toContain('./docker/devforge-realtime/terminal-utils.js:/terminal/terminal-utils.js');
})->with([
...
it('keeps realtime terminal server logging behind the explicit debug flag', function () {
    $terminalServer = file_get_contents(base_path('docker/devforge-realtime/terminal-server.js'));
