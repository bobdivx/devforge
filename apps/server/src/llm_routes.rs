...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/api/v1/llm/providers/{id}/activate", post(activate_provider))
        ...
        .route("/api/v1/llm/connect", post(llm_connect).delete(llm_disconnect))
...
const SELECT_PROVIDERS: &str = "SELECT id, catalog_id, name, provider, api_key, base_url, model, is_default, enabled, priority, COALESCE(healthy, 1) as healthy, COALESCE(last_probe_at, '') as last_probe_at, COALESCE(last_probe_error, '') as last_probe_error, COALESCE(resolved_model, '') as resolved_model FROM llm_providers ORDER BY priority ASC, name ASC";

const SELECT_ONE: &str = "SELECT id, catalog_id, name, provider, api_key, base_url, model, is_default, enabled, priority, COALESCE(healthy, 1) as healthy, COALESCE(last_probe_at, '') as last_probe_at, COALESCE(last_probe_error, '') as last_probe_error, COALESCE(resolved_model, '') as resolved_model FROM llm_providers WHERE id = ?";
...
async fn list_providers(
    ...
    // Bootstrap depuis instance_settings si table vide.
    if rows.is_empty() {
        ...
        if let Some((prov, key, model, base)) = legacy {
            if prov != "stub" && (!key.is_empty() || !base.is_empty() || prov == "ollama") {
                ...
                let _ = sqlx::query(
                    r#"INSERT INTO llm_providers (id, catalog_id, name, provider, api_key, base_url, model, is_default, enabled, priority, created_at, updated_at)
                       VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, 0, ?, ?)"#,
                )
                .bind(&id)
                .bind(&prov)
                ...
                return Ok(Json(json!({
                    ...
                    "active_mode": state.backends.llm_mode(),
                })));
            }
        }
    }
...
async fn upsert_provider(
    ...
    let next_priority: i64 = if prefer_first {
        ...
        let max: Option<(i64,)> =
            sqlx::query_as("SELECT COALESCE(MAX(priority), -1) FROM llm_providers")
                .fetch_optional(&state.pool)
                .await
                .ok()
                .flatten();
    ...
    let id = if let Some(existing) = body.id.filter(|s| !s.is_empty()) {
        ...
        if api_key.trim().is_empty() {
            let stored: Option<(String,)> =
                sqlx::query_as("SELECT api_key FROM llm_providers WHERE id = ?")
                    .bind(&existing)
                    .fetch_optional(&state.pool)
                    .await
                    .ok()
        ...
        let prio: i64 = if prefer_first {
            ...
            let cur: Option<(i64,)> =
                sqlx::query_as("SELECT priority FROM llm_providers WHERE id = ?")
                    .bind(&existing)
                    .fetch_optional(&state.pool)
                    .await
                    .ok()
        ...
        if prefer_first {
            ...
            sqlx::query("UPDATE llm_providers SET priority = priority + 1 WHERE id != ?")
                .bind(&existing)
                .execute(&state.pool)
                .await
                .ok();
        ...
        sqlx::query(
            r#"UPDATE llm_providers SET catalog_id=?, name=?, provider=?, api_key=?, base_url=?, model=?, is_default=?, enabled=?, priority=?, updated_at=? WHERE id=?"#,
        )
        .bind(&catalog_id)
        .bind(&name)
        .bind(&provider)
        ...
        let id = uuid::Uuid::new_v4().to_string();
        if prefer_first {
            sqlx::query("UPDATE llm_providers SET priority = priority + 1")
                .execute(&state.pool)
                .await
                .ok();
        }
    ...
    Ok(Json(json!({ "data": present(&row), "active_mode": state.backends.llm_mode() })))
}
...
async fn activate_row(
    ...
    // Passe en tête de priorité (0) et décale les autres
    sqlx::query("UPDATE llm_providers SET priority = priority + 1 WHERE id != ?")
        .bind(id)
        .execute(&state.pool)
        .await
    ...
    sqlx::query(
        "UPDATE llm_providers SET priority = 0, is_default = 1, enabled = 1, updated_at = ? WHERE id = ?",
    )
    .bind(now_str())
    .bind(id)
    .execute(&state.pool)
    ...
    sqlx::query("UPDATE llm_providers SET is_default = 0 WHERE id != ?")
        .bind(id)
        .execute(&state.pool)
        .await
        .ok();
...
async fn delete_provider(
    ...
    let remaining: Option<(i64,)> =
        sqlx::query_as("SELECT COUNT(*) FROM llm_providers WHERE enabled = 1")
            .fetch_optional(&state.pool)
            .await
            .ok()
            .flatten();
    if remaining == Some((0,)) || remaining.is_none() {
        let _ = state.configure_llm("stub", "", "gpt-4o-mini", "").await;
    } else {
        let _ = state.reload_llm_chain().await;
    }
    Ok(Json(json!({"ok": true, "active_mode": state.backends.llm_mode()})))
}
...
async fn probe_all_providers(
    ...
    require_admin(&state, &headers).await?;
    // reload_llm_chain probe chaque enabled et filtre la chaîne
    state.reload_llm_chain().await.map_err(|e| {
        (
            axum::http::StatusCode::BAD_REQUEST,
            Json(json!({"error": e.to_string()})),
    ...
    let healthy = results.iter().filter(|r| r["ok"].as_bool() == Some(true)).count();
    Ok(Json(json!({
        "ok": true,
        "healthy": healthy,
        "total": results.len(),
...
async fn reorder_providers(
    ...
    for (i, id) in body.ordered_ids.iter().enumerate() {
        sqlx::query("UPDATE llm_providers SET priority = ?, is_default = ?, updated_at = ? WHERE id = ?")
            .bind(i as i64)
...
async fn llm_status(
    ...
    let (provider, key, model, base) =
        row.unwrap_or_else(|| ("auto".into(), String::new(), "gpt-4o-mini".into(), String::new()));
...
async fn llm_connect(
    ...
    if api_key.trim().is_empty() {
        let stored: Option<(String,)> =
            sqlx::query_as("SELECT llm_api_key FROM instance_settings WHERE id = 1")
                .fetch_optional(&state.pool)
                .await
                .map_err(|e| {
                    (
...
async fn llm_disconnect(
    ...
    require_admin(&state, &headers).await?;
    state
        .configure_llm("stub", "", "gpt-4o-mini", "")
        .await
        .map_err(|e| {
            (
                axum::http::StatusCode::BAD_REQUEST,
...
async fn llm_list_models(
    ...
    if api_key.trim().is_empty() || base_url.is_empty() {
        let row: Option<(String, String)> = sqlx::query_as(
            "SELECT llm_api_key, llm_base_url FROM instance_settings WHERE id = 1",
        )
        .fetch_optional(&state.pool)
        .await
        .map_err(|e| {
        ...
        && provider != "ollama"
    {
        return Err((
            axum::http::StatusCode::BAD_REQUEST,
            Json(json!({"error": format!("Une clé API est requise pour lister les modèles {provider}.")})),
        ));
    }
