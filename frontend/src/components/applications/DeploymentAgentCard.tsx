    ...
    return (
        <div class="mb-3 grid min-w-0 gap-2 sm:gap-3 overflow-hidden rounded-lg border border-base-300/60 bg-base-100 p-3">
            <div class="flex min-w-0 flex-wrap items-start justify-between gap-2">
                ...
                <span class={`shrink-0 rounded-full border px-2 py-0.5 text-[11px] font-medium ${outcomeToneClass(correction.outcome)}`}>
                    ...
                    {correction.pills.map((pill) => {
                        ...
                        if (pill.active && href) {
                            return (
                                <a
                                    key={pill.id}
                                    class={`inline-flex max-w-full items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-medium ${className}`}
                                    href={href}
                                    target={isExternal ? '_blank' : undefined}
                                    rel={isExternal ? 'noreferrer' : undefined}
                                >
                                    <span class="truncate">{pill.label}</span>
                                    {detail && <span class="shrink-0 font-mono opacity-80">{detail}</span>}
                                    {isExternal && <ExternalLink class="size-3 shrink-0" aria-hidden />}
                                    {!isExternal && <ArrowRight class="size-3 shrink-0" aria-hidden />}
                                </a>
                            );
                        }
                        ...
                        return (
                            ...
                            >
                                <span class="truncate">{pill.label}</span>
                                {pill.active && detail && <span class="shrink-0 font-mono opacity-80">{detail}</span>}
                            </span>
                        );
                    })}
                </div>
            ...
            {Array.isArray(correction.steps) && correction.steps.length > 0 && (
                <ol class="grid list-decimal gap-1.5 break-words pl-4 text-xs text-base-content/75">
            ...
            {correction.actions.length > 0 && (
                <ul class="grid min-w-0 gap-1.5 text-xs text-base-content/70">
                    {correction.actions.map((action, index) => (
                        <li key={`${action.kind}-${index}`} class="flex min-w-0 flex-wrap items-center justify-between gap-2">
                            ...
                            {action.pr_url && (
                                <a class="link link-primary shrink-0 text-[11px]" href={action.pr_url} target="_blank" rel="noreferrer">
                                    Ouvrir la PR
                                </a>
                            )}
                            {action.commit_url && !action.pr_url && (
                                <a class="link link-primary shrink-0 text-[11px]" href={action.commit_url} target="_blank" rel="noreferrer">
                                    Voir le commit
                                </a>
                            )}
                        </li>
    ...
    return (
        <article class="min-w-0 overflow-hidden rounded-xl border border-base-300/70 bg-base-200/20 p-3 sm:p-4">
                ...
                <div class="mb-3 min-w-0">
                    ...
                    <ul class="grid min-w-0 gap-2">
                        {run.actions_taken.map((action, index) => (
                            <li
                                class="flex min-w-0 flex-wrap items-center justify-between gap-2 overflow-hidden rounded-lg border border-base-300/60 bg-base-100 px-3 py-2 text-xs"
                                key={`${action.at}-${index}`}
                            >
            ...
            {(subagents.length > 0 || ephemeralTasks.length > 0) && (
                <div class="mb-3 min-w-0 overflow-hidden rounded-lg border border-base-300/60 bg-base-100/80 p-3">
                    <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-base-content/45">Sous-agents</p>
                    <ul class="grid min-w-0 gap-2">
                        {subagents.map((sub) => (
                            <li
                                class="grid min-w-0 gap-2 overflow-hidden rounded-lg border border-base-300/50 px-3 py-2 text-xs sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start"
                                key={sub.uuid}
                            >
                                <div class="min-w-0 overflow-hidden">
                                    <p class="break-words font-medium">{sub.child_agent?.name ?? 'Sous-agent'}</p>
                                    {sub.reason && (
                                        <p class="mt-0.5 break-words text-base-content/45">{sub.reason}</p>
                                    )}
                                </div>
                                <div class="shrink-0 justify-self-start sm:justify-self-end">
                        ...
                        {ephemeralTasks.map((task, index) => (
                            <li
                                class="min-w-0 overflow-hidden rounded-lg border border-base-300/50 px-3 py-2 text-xs"
                                key={`ephemeral-${index}`}
                            >
                                <p class="break-words font-medium">{task.model_label ?? 'Tâche'}</p>
                                {task.goal && (
                                    <p class="mt-1 line-clamp-3 break-words text-base-content/55">{task.goal}</p>
                                )}
                            </li>
                        ))}
                    </ul>
            ...
            {run.logs && (
                <div class="min-w-0 overflow-hidden rounded-lg border border-base-300/50">
                    <button
                        type="button"
                        class="flex w-full min-w-0 items-center justify-between gap-2 px-3 py-2.5 text-left text-xs font-medium text-base-content/65 hover:bg-base-200/50"
                        aria-expanded={logsOpen}
                        onClick={() => setLogsOpen((open) => !open)}
                    >
                        <span>Logs</span>
                        <ChevronDown class={`size-3.5 shrink-0 transition ${logsOpen ? 'rotate-180' : ''}`} aria-hidden />
                    </button>
                    {logsOpen && (
                        <div class="min-w-0 overflow-hidden border-t border-base-300/50 p-2">
