...
describe('DatabaseEnvironmentVariablesPanel', () => {
    ...
    it('préremplit la valeur réelle dans le formulaire de modification', async () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: 'Modifier POSTGRES_DB' }));

        expect(await screen.findByRole('heading', { name: 'Modifier POSTGRES_DB' })).toBeInTheDocument();
        expect(await screen.findByDisplayValue('secret-db')).toBeInTheDocument();
