...
impl DatabaseFacade {
    ...
    pub async fn provision(&self, name: &str, engine: Option<&str>) -> Value {
        ...
        if engine != "sqlite" {
            return json!({
                "ok": false,
                "error": format!("Moteur « {engine} » non pris en charge. Le provisionnement DevForge crée un fichier SQLite."),
                "engine": engine
            });
        }
    ...
    pub fn status(&self, database_uuid: &str) -> Value {
        ...
        match std::fs::read_to_string(&path) {
            Ok(raw) => serde_json::from_str(&raw).unwrap_or_else(|_| {
                json!({ "ok": false, "uuid": database_uuid, "error": "catalogue illisible" })
            }),
            Err(_) => json!({
                "ok": false,
                "uuid": database_uuid,
...
pub async fn provision_sqlite_file(path: &Path) -> Result<String, String> {
    ...
    .map_err(|e| e.to_string())?;
    sqlx::query(
        "INSERT OR IGNORE INTO _devforge_db (key, value) VALUES ('engine', 'sqlite')",
    )
    .execute(&pool)
    .await
    .map_err(|e| e.to_string())?;
...
fn write_meta(root: &Path, uuid: &str, meta: &Value) -> Result<(), String> {
    ...
    std::fs::write(&path, serde_json::to_vec_pretty(meta).map_err(|e| e.to_string())?)
        .map_err(|e| e.to_string())
}
